...
func _ready():
	...
	scroll.get_h_scroll_bar().custom_minimum_size.y = 24
	...
	d.connect("connection_state_changed",Callable(self,"_connection_changed"))
	d.connect("status_changed",Callable(self,"_status_changed"))
	d.connect("server_settings_received",Callable(self,"_server_settings_received"))
	update_values()
...
func _resize_for_mobile():
	if G.IsMobile:
		...
		for n in nodes:
			if n is Control:
				n.custom_minimum_size = Vector2(n.custom_minimum_size.x, 64 if n.custom_minimum_size.y < 64 else n.custom_minimum_size.y)
...
func _init_point():
	if G.Billings:
		await G.Billings.billings_ready
		donations.visible = G.Billings != null
		#empty_top.visible = G.Billings == null
		G.Billings.connect("points_updated",Callable(self,"_points_update"))
...
func _on_GRSettings_resized():
	var cols = 2 if (size.x / size.y > 1.3) else 1
...
func _on_GRSettings_visibility_changed():
	...
	if visible:
		...
		GodotRemote.get_output_device().capture_input = false
	else:
		# yield needs to wait next frame and not instant close app
		await get_tree().idle_frame
		get_tree().set_quit_on_go_back(true)
		GodotRemote.get_output_device().capture_input = true
...
func update_values():
	...
	filtering.button_pressed = G.texture_filtering
	password.text = G.password
	sync_orient.button_pressed = G.sync_viewport_orientation
	sync_aspect.button_pressed = G.sync_viewport_aspect_ratio
	keepscreen.button_pressed = G.keepscreenon
	captureinput.button_pressed = G.capture_input_when_custom_scene
	touches_to_open.selected = G.TouchesToOpenSettings - 3
	
	enabled_server_settings.button_pressed = G.override_server_settings
	sync_server_settings.button_pressed = G.sync_server_settings
	...
	video_stream.button_pressed = G.server_video_stream
...
func _server_settings_received(_settings : Dictionary):
	...
	for kk in k:
		...
		match kk:
			...
			C.GRDevice_SERVER_PARAM_VIDEO_STREAM_ENABLED: video_stream.button_pressed = v
...
func _on_button_disable_Timer_timeout():
	var _status = GodotRemote.get_output_device().get_status()
	_set_buttons_disabled(_status == C.GRDevice_STATUS_STARTING or _status == C.GRDevice_STATUS_STOPPING)

func _update_start_stop():
	match GodotRemote.get_output_device().get_status():
...
func _on_Device_ID_text_changed(new_text : String):
	if !new_text.is_empty():
		GodotRemote.get_output_device().device_id = new_text
		G.device_id = new_text
	else:
		G.device_id = G.get_random_hash()
		GodotRemote.get_output_device().device_id = G.device_id
...
func _on_wifi_SetAddress_pressed():
	_disable_buttons_by_timer()
	GodotRemote.get_output_device().connection_type = con_type_menu.selected
	var address = wifi_ip_line.text.replace("http://", "").replace("https://", "").strip_edges()
	if GodotRemote.get_output_device().set_address_port(address, wifi_port_line.value):
		G.ip = address
...
func _on_adb_SetAddress_pressed():
	_disable_buttons_by_timer()
	GodotRemote.get_output_device().connection_type = con_type_menu.selected
	GodotRemote.get_output_device().port = adb_port_line.value
...
func _on_stretch_Type_item_selected(index):
	GodotRemote.get_output_device().stretch_mode = index
...
func _on_CaptureInput_toggled(button_pressed: bool) -> void:
	...
	if GodotRemote.get_output_device().get_custom_input_scene():
		GodotRemote.get_output_device().capture_pointer = button_pressed
...
func _on_override_settings_State_toggled(button_pressed):
	...
	if button_pressed:
		...
		GodotRemote.get_output_device().disable_overriding_server_settings()

func _on_SyncServerSettings_toggled(button_pressed):
	G.sync_server_settings = button_pressed
	GodotRemote.get_output_device().server_settings_syncing = button_pressed

func _on_server_Quality_value_changed(value):
	if not updated_by_code:
		if G.override_server_settings:
			GodotRemote.get_output_device().set_server_setting(C.GRDevice_SERVER_PARAM_JPG_QUALITY, value)
	G.server_jpg_quality = value

func _on_server_render_Scale_value_changed(value):
	if not updated_by_code:
		if G.override_server_settings:
			GodotRemote.get_output_device().set_server_setting(C.GRDevice_SERVER_PARAM_RENDER_SCALE, value)
	G.server_render_scale = value

func _on_server_send_FPS_value_changed(value):
	if not updated_by_code:
		if G.override_server_settings:
			GodotRemote.get_output_device().set_server_setting(C.GRDevice_SERVER_PARAM_SKIP_FRAMES, value)
	G.server_skip_fps = value

func _on_compression_type_item_selected(index):
	if not updated_by_code:
		if G.override_server_settings:
			GodotRemote.get_output_device().set_server_setting(C.GRDevice_SERVER_PARAM_COMPRESSION_TYPE, index)
	G.server_compression_type = index

func _on_video_stream_Enabled_toggled(button_pressed):
	if not updated_by_code:
		if G.override_server_settings:
			GodotRemote.get_output_device().set_server_setting(C.GRDevice_SERVER_PARAM_VIDEO_STREAM_ENABLED, button_pressed)
