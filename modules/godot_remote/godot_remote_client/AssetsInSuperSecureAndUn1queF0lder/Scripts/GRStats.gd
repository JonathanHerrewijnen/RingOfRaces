...
func _ready():
	G.connect("show_stats_changed",Callable(self,"show_stats_changed"))
	show_stats_changed(G.show_stats)

func _process(_delta):
	if GodotRemote.get_output_device():
		if stat_state > G.StatInfoState.Hidden:
			fps.text = "FPS: %.1f" % GodotRemote.get_output_device().get_avg_fps()
			ping.text = "PING: %.1f" % GodotRemote.get_output_device().get_avg_ping()
		
		if stat_state > G.StatInfoState.Simple:
			fps_min.text = "%.1f" % GodotRemote.get_output_device().get_min_fps()
			fps_max.text = "%.1f" % GodotRemote.get_output_device().get_max_fps()
			
			ping_min.text = "%.1f" % GodotRemote.get_output_device().get_min_ping()
			ping_max.text = "%.1f" % GodotRemote.get_output_device().get_max_ping()
