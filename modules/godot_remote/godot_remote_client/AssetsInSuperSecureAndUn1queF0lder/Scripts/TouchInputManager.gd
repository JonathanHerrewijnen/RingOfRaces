...
var InputPopup : CustomPopupTextInput : set = _set_input_popup
...
func _enter_tree():
	if not Engine.editor_hint:
		get_tree().connect("node_added",Callable(self,"_node_added_to_scene"))
		get_tree().connect("node_removed",Callable(self,"_node_removed_from_scene"))
...
func _set_input_popup(val):
	InputPopup = val
	InputPopup.connect("visibility_changed",Callable(self,"_input_visibility_changed"))
...
func _register_line_edit(line : LineEdit, title : String):
	...
	#line.editable = false
	line.connect("gui_input",Callable(self,"_on_touch_line_edit_gui_input").bind(line))
...
func _on_touch_line_edit_gui_input(event : InputEvent, line : LineEdit):
	if event is InputEventMouseButton:
		if event.button_index == MOUSE_BUTTON_LEFT:
			if not event.pressed and Rect2(Vector2(), line.size).intersects(Rect2(event.position, Vector2())):
